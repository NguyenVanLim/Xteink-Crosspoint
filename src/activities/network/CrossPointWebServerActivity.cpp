...
void CrossPointWebServerActivity::onNetworkModeSelected(
  ...
  const char *modeName = "Tham gia mạng";
  if (mode == NetworkMode::CONNECT_CALIBRE) {
    modeName = "Kết nối với Calibre";
  } else if (mode == NetworkMode::CREATE_HOTSPOT) {
    modeName = "Tạo điểm phát sóng";
  }
  ...
  if (mode == NetworkMode::CONNECT_CALIBRE) {
    ...
    enterNewActivity(new CalibreConnectActivity(renderer, mappedInput, [this] {
      ...
      enterNewActivity(new NetworkModeSelectionActivity(
          renderer, mappedInput,
          [this](const NetworkMode nextMode) {
            onNetworkModeSelected(nextMode);
          },
          [this]() { onGoBack(); }));
    }));
    return;
...
void CrossPointWebServerActivity::render() const {
  ...
  if (state == WebServerActivityState::SERVER_RUNNING) {
    ...
    renderer.drawCenteredText(UI_12_FONT_ID, pageHeight / 2 - 20,
                              "Đang khởi tạo...", true, EpdFontFamily::BOLD);
    renderer.displayBuffer();
  }
}
...
void CrossPointWebServerActivity::renderServerRunning() const {
  ...
  renderer.drawCenteredText(UI_12_FONT_ID, 15, "Chuyển file", true,
                            EpdFontFamily::BOLD);
  ...
  if (isApMode) {
    ...
    renderer.drawCenteredText(UI_10_FONT_ID, startY, "Chế độ điểm phát sóng",
    ...
    renderer.drawCenteredText(SMALL_FONT_ID, startY + LINE_SPACING * 2,
                              "Kết nối thiết bị của bạn vào Wifi này");

    renderer.drawCenteredText(
        SMALL_FONT_ID, startY + LINE_SPACING * 3,
        "hoặc quét mã QR bằng điện thoại để kết nối Wifi.");
    ...
    std::string ipUrl = "hoặc http://" + connectedIP + "/";
    ...
    std::string ipInfo = "Địa chỉ IP: " + connectedIP;
    renderer.drawCenteredText(UI_10_FONT_ID, startY + LINE_SPACING,
                              ipInfo.c_str());
    ...
    std::string hostnameUrl =
        std::string("hoặc http://") + AP_HOSTNAME + ".local/";
